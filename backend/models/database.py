...
def update_orders_from_target(orders_data: List[Dict[str, Any]], target_order_id: str, raw_orders_data: List[Dict[str, Any]] = None):
    """从目标订单ID开始更新订单数据"""
    ...
    try:
        for i, order in enumerate(orders_data):
            ...
            # 检查订单是否已存在
            cursor.execute('SELECT COUNT(*) FROM order_products WHERE 订单编号 = ?', (order_id,))
        ...
        message_parts = []
        if saved_records > 0:
            message_parts.append(f"新增 {saved_records} 个订单")
        if updated_records > 0:
            message_parts.append(f"更新 {updated_records} 个订单")
        if orders_need_details_count > 0:
            message_parts.append(f"发现 {orders_need_details_count} 个订单需要详细显示")
        ...
        return {
            ...
            "orders_need_details_count": orders_need_details_count,
            "message": "，".join(message_parts) if message_parts else "没有符合条件的订单需要处理"
        }
